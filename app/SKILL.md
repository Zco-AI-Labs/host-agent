...
- **Contextualized Query Delegation**: When calling `consultAgent`, always enrich your `query` parameter with the active workspace persona identity, venue names, or key entity keywords from the conversation context so the subagent receives a fully specified, domain-grounded query (e.g. `consultAgent(agentId="knowledge_agent", query="upcoming TD Garden events")` instead of `query="upcoming events"`).
